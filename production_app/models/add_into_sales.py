        ...
        for record in self:
            ...
            for lin in create_in_inv.orderLines_ids:
                vals = {
                    ...
                    'price_subtotal': lin.price_subtotal,
                ...
                for recs in old_credit.invoice_line_ids:
                    for new_rec in new_invoice.invoice_line_ids:
                        if lin.name == recs.name and lin.name == new_rec.name:
                            ...
                            # calacultor adiscount
                            if lin.price_subtotal == 0 or lin.quantity == 0:
                                new_discount = 0
                            else:
                                new_discount = (
                                                           100 * to_be_subtotal - 100 * lin.price_subtotal * lin.quantity) / -lin.price_subtotal * lin.quantity * 1
                            print(lin.name)
    ...
    def amount_all(self):
        for single_rec in self:
            ...
            for rec in single_rec.order_line:
                dicAmnt = rec.linediscPerct / 100 * rec.price_subtotal * rec.product_uom_qty
                disc = rec.discount / 100 * rec.price_subtotal * rec.product_uom_qty
                subtot = rec.price_subtotal * rec.product_uom_qty - dicAmnt - disc
